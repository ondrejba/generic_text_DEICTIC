...
def build_getMoveActionDescriptors(make_obs_ph, actionShape, actionShapeSmall, stride):
    ...
    patchesTiled = tf.reshape(patches,
                              [patchesShape[0] * patchesShape[1] * patchesShape[2], actionShape[0], actionShape[1], 1])
...
def main(initEnvStride, envStride, fileIn, fileOut, inputmaxtimesteps):
    ...
    def make_actionDeic_ph(name):
        return U.BatchInput((descriptorShapeSmall[0] * descriptorShapeSmall[1] * descriptorShapeSmall[2],), name=name)
    ...
    getMoveActionDescriptors = build_getMoveActionDescriptors(make_obs_ph=make_obs_ph, actionShape=descriptorShape,
                                                              actionShapeSmall=descriptorShapeSmall, stride=env.stride)
    ...
    for t in range(max_timesteps):
        ...
        actionsPickDescriptors = np.reshape(actionsPickDescriptors, (actionsPickDescriptors.shape[0], -1))
        actionsPlaceDescriptors = np.reshape(actionsPlaceDescriptors, (actionsPlaceDescriptors.shape[0], -1))
        ...
        qCurr = np.concatenate(
            [np.r_[qCurrNotHoldingPick, qCurrNotHoldingPlace], np.r_[qCurrHoldingPick, qCurrHoldingPlace]], axis=1)
        ...
        qCurrNoise = qCurr + np.random.random(
            np.shape(qCurr)) * 0.01  # add small amount of noise to break ties randomly
        ...
        replay_buffer.add(cp.copy(obs[1]), np.copy(actionDescriptors[action, :]), cp.copy(rew), cp.copy(new_obs[1]),
                          cp.copy(float(done)))
        ...
        if t > learning_starts and t % train_freq == 0:
            ...
            # Train
            targetTrainNotHolding(actionPatches, np.reshape(qCurrTarget[:, 0], [batch_size, 1]),
                                  np.reshape(weights, [batch_size, 1]))
            targetTrainHolding(actionPatches, np.reshape(qCurrTarget[:, 1], [batch_size, 1]),
                               np.reshape(weights, [batch_size, 1]))
        ...
        if done and print_freq is not None and len(episode_rewards) % print_freq == 0:
            ...
            print("steps: " + str(t) + ", episodes: " + str(num_episodes) + ", mean 100 episode reward: " + str(
                mean_100ep_reward) + ", % time spent exploring: " + str(
                int(100 * exploration.value(t))) + ", time elapsed: " + str(timerFinal - timerStart))
