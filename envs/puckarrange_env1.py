...
#
#  This version of the environment allows the initial configuration to be off-grid.
#
import math
import numpy as np
import gym
...
#        initMoveCenters = range(int(self.blockSize/2),int(self.gridSize-(self.blockSize/2)+1),1)
        initMoveCenters = range(int(self.blockSize/2),int(self.gridSize-(self.blockSize/2)+1),self.stride)

        # Initialize state as null
        self.state = []

        self.state.append(np.zeros(self.observation_space.spaces[0].shape)) # self.state[0] encodes block layout
